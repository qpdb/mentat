...
use core_traits::{Binding, StructuredMap};

// TODO: re-export from Mentat.
use mentat_core::ValueRc;

use mentat::{
    Entid, HasSchema, IntoResult, Keyword, Pullable, QueryInputs, Queryable, RelResult, Store,
    TypedValue,
};
...
fn test_simple_pull() {
    ...
    let mut store = Store::open("").expect("opened");
    {
        ...
        beacon = *report
            .tempids
            .get(&"a17592186045471".to_string())
            .expect("beacon");
        beacon_district = *report
            .tempids
            .get(&"a17592186045450".to_string())
            .expect("beacon district");

        capitol = *report
            .tempids
            .get(&"a17592186045439".to_string())
            .expect("capitol");
        capitol_district = *report
            .tempids
            .get(&"a17592186045438".to_string())
            .expect("capitol district");
    ...
    let district = schema
        .get_entid(&kw!(:neighborhood/district))
        .expect("district")
        .0;
    ...
    let hoods: BTreeSet<Entid> = store
        .q_once(query, None)
        .into_coll_result()
        .expect("hoods")
        .into_iter()
    ...
    let c: StructuredMap = vec![
        ...
        (
            ...
            TypedValue::Ref(capitol_district),
        ),
    ]
    .into();
    let b: StructuredMap = vec![
        ...
        (
            kw!(:neighborhood/district),
            TypedValue::Ref(beacon_district),
        ),
    ]
    .into();
    ...
    let query = r#"[:find ?hood (pull ?district [:db/id
                    ...
                    :order ?hood]"#;
    let results: RelResult<Binding> = store
        .begin_read()
        .expect("read")
        .q_once(query, None)
        .into_rel_result()
    ...
    let beacon_district_pull: Vec<(Keyword, TypedValue)> = vec![
        ...
        (
            kw!(:district/region),
            schema
                .get_entid(&Keyword::namespaced("region", "se"))
                .unwrap()
                .into(),
        ),
    ];
    ...
    let capitol_district_pull: Vec<(Keyword, TypedValue)> = vec![
        ...
        (
            kw!(:district/region),
            schema
                .get_entid(&Keyword::namespaced("region", "e"))
                .unwrap()
                .into(),
        ),
    ];
    ...
    assert_eq!(
        prepared.run(None).into_rel_result().expect("results"),
        expected
    );
    ...
    let result = reader
        .q_once(
            query,
            QueryInputs::with_value_sequence(vec![(var!(?hood), TypedValue::Ref(beacon))]),
        )
        .into_scalar_result()
        .expect("success")
        .expect("result");
    ...
    let expected: Vec<StructuredMap> = vec![
        vec![
            (kw!(:district/name), TypedValue::from("East")),
            (kw!(:district/region), TypedValue::Ref(65556)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Southwest")),
            (kw!(:district/region), TypedValue::Ref(65559)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Downtown")),
            (kw!(:district/region), TypedValue::Ref(65560)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Greater Duwamish")),
            (kw!(:district/region), TypedValue::Ref(65557)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Ballard")),
            (kw!(:district/region), TypedValue::Ref(65561)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Northeast")),
            (kw!(:district/region), TypedValue::Ref(65555)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Southeast")),
            (kw!(:district/region), TypedValue::Ref(65557)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Northwest")),
            (kw!(:district/region), TypedValue::Ref(65561)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Central")),
            (kw!(:district/region), TypedValue::Ref(65556)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Delridge")),
            (kw!(:district/region), TypedValue::Ref(65559)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Lake Union")),
            (kw!(:district/region), TypedValue::Ref(65560)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("Magnolia/Queen Anne")),
            (kw!(:district/region), TypedValue::Ref(65560)),
        ]
        .into(),
        vec![
            (kw!(:district/name), TypedValue::from("North")),
            (kw!(:district/region), TypedValue::Ref(65555)),
        ]
        .into(),
    ];
    ...
    let expected: Vec<StructuredMap> = vec![
        vec![
            (
                kw!(:community/name),
                Binding::Scalar(TypedValue::from("Community Harvest of Southwest Seattle")),
            ),
            (
                kw!(:community/category),
                vec![Binding::Scalar(TypedValue::from("sustainable food"))].into(),
            ),
        ]
        .into(),
        vec![
            (
                kw!(:community/name),
                Binding::Scalar(TypedValue::from("InBallard")),
            ),
            (
                kw!(:community/category),
                vec![
                    Binding::Scalar(TypedValue::from("shopping")),
                    Binding::Scalar(TypedValue::from("food")),
                    Binding::Scalar(TypedValue::from("nightlife")),
                    Binding::Scalar(TypedValue::from("services")),
                ]
                .into(),
            ),
        ]
        ...
        vec![
            (
                kw!(:community/name),
                Binding::Scalar(TypedValue::from("Seattle Chinatown Guide")),
            ),
            (
                kw!(:community/category),
                vec![
        ...
        vec![
            (
                kw!(:community/name),
                Binding::Scalar(TypedValue::from("University District Food Bank")),
            ),
            (
                kw!(:community/category),
                vec![Binding::Scalar(TypedValue::from("food bank"))].into(),
            ),
        ]
        .into(),
    ];
