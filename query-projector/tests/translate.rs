...
use edn::query::{FindSpec, Keyword, Variable};
...
use mentat_query_algebrizer::{
    algebrize, algebrize_with_inputs, parse_find_string, Known, QueryInputs,
};

use mentat_query_projector::ConstantProjector;
...
fn query_to_sql(query: ProjectedSelect) -> SQLQuery {
    match query {
        ...
        ProjectedSelect::Constant(constant) => {
            panic!(
                "ProjectedSelect wasn't ::Query! Got constant {:#?}",
...
fn translate_with_inputs_to_constant(
    schema: &Schema,
    query: &'static str,
    inputs: QueryInputs,
) -> ConstantProjector {
...
fn make_arg(name: &'static str, value: &'static str) -> (String, Rc<rusqlite::types::Value>) {
    (
        name.to_string(),
        Rc::new(rusqlite::types::Value::Text(value.to_string())),
    )
}
...
fn test_unbound_variable_limit() {
    ...
    // later input.
    let query =
        r#"[:find ?x :in ?limit-is-9-great :where [?x :foo/bar "yyy"] :limit ?limit-is-9-great]"#;
    let SQLQuery { sql, args } = translate_with_inputs(&schema, query, QueryInputs::default());
    assert_eq!(
        sql,
        "SELECT DISTINCT `datoms00`.e AS `?x` \
...
fn test_bound_variable_limit() {
    ...
    let inputs = QueryInputs::with_value_sequence(vec![(
        Variable::from_valid_name("?limit"),
        TypedValue::Long(92),
    )]);
...
fn test_bound_variable_limit_affects_types() {
    ...
    assert_eq!(
        ...
        algebrized
            .cc
            .known_type(&Variable::from_valid_name("?limit"))
    );
...
fn test_unknown_attribute_integer_value() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `datoms00`.e AS `?x` FROM `datoms` AS `datoms00` WHERE `datoms00`.v = -1"
    );
    assert_eq!(args, vec![]);
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `datoms00`.e AS `?x` FROM `datoms` AS `datoms00` WHERE `datoms00`.v = 2"
    );
    assert_eq!(args, vec![]);
}
...
fn test_type_required_string() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `all_datoms00`.e AS `?x` \
...
fn test_compare_long_to_double_constants() {
    ...
    assert_eq!(
        sql,
        "SELECT `datoms00`.e AS `?e` FROM `datoms` AS `datoms00` \
         WHERE `datoms00`.a = 99 \
...
fn test_simple_or_join() {
    ...
    assert_eq!(
        args,
        vec![make_arg("$v0", "http://foo.com/"), make_arg("$v1", "Foo")]
    );
}
...
fn test_not_join() {
    ...
    associate_ident(
        &mut schema,
        Keyword::namespaced("bookmarks", "date_created"),
        99,
    );
    add_attribute(
        &mut schema,
...
fn test_with_without_aggregate() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `datoms00`.e AS `?x` FROM `datoms` AS `datoms00` WHERE `datoms00`.a = 99"
    );
    assert_eq!(args, vec![]);
...
fn test_order_by() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `datoms00`.e AS `?x`, `datoms00`.v AS `?y` \
         FROM `datoms` AS `datoms00` \
         WHERE `datoms00`.a = 99 \
         ORDER BY `?y` DESC"
    );
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `all_datoms00`.e AS `?x`, `all_datoms00`.v AS `?y`, \
...
fn test_ground_scalar() {
    ...
    let inputs =
        QueryInputs::with_value_sequence(vec![(Variable::from_valid_name("?v"), "aaa".into())]);
...
fn test_ground_tuple() {
    ...
    let constant = translate_to_constant(&schema, query);
    assert_eq!(
        constant.project_without_rows().unwrap().into_rel().unwrap(),
    ...
    let inputs = QueryInputs::with_value_sequence(vec![
        (Variable::from_valid_name("?u"), TypedValue::Long(2)),
    ...
    assert_eq!(
        ...
        Some(vec![
            ...
            TypedValue::typed_string("aaa").into()
        ])
    );
...
fn test_ground_coll() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `c00`.`?x` AS `?x` FROM \
         (SELECT 0 AS `?x` WHERE 0 UNION ALL VALUES ($v0), ($v1)) AS `c00`"
    );
    assert_eq!(args, vec![make_arg("$v0", "xxx"), make_arg("$v1", "yyy")]);
...
fn test_ground_rel() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `c00`.`?x` AS `?x`, `c00`.`?y` AS `?y` FROM \
    ...
    let inputs = QueryInputs::with_value_sequence(vec![
        (Variable::from_valid_name("?u"), TypedValue::Long(3)),
...
fn test_compound_with_ground() {
    ...
    assert_eq!(args, vec![make_arg("$v0", "yyy"), make_arg("$v1", "zzz"),]);
    ...
    assert_eq!(
        sql,
        "SELECT $v0 AS `?x` FROM `datoms` AS `datoms00` \
...
fn test_fulltext() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `datoms01`.e AS `?entity`, \
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `datoms00`.e AS `?entity`, \
...
fn test_fulltext_inputs() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `fulltext_values00`.text AS `?val` \
         FROM \
    ...
    let inputs = QueryInputs::with_value_sequence(vec![(
        Variable::from_valid_name("?entity"),
        TypedValue::Ref(111),
    )]);
    let SQLQuery { sql, args } = translate_with_inputs(&schema, query, inputs);
    assert_eq!(
...
fn test_instant_range() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `datoms00`.e AS `?e` \
         FROM \
         `datoms` AS `datoms00` \
         WHERE `datoms00`.a = 99 \
...
fn test_tx_ids() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `transactions00`.tx AS `?tx` \
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `transactions02`.tx AS `?tx` \
         FROM `datoms` AS `datoms00`, \
         `datoms` AS `datoms01`, \
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `all_datoms01`.e AS `?e`, \
         `all_datoms01`.a AS `?a`, \
         `all_datoms01`.v AS `?v`, \
...
fn test_tx_data() {
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `transactions00`.e AS `?e`, \
    ...
    assert_eq!(
        sql,
        "SELECT DISTINCT `transactions01`.e AS `?e`, \
